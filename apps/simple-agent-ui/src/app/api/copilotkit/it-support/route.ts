...
// STEP 1: Initialize HTTP Agent for IT Support Backend
// Create agent connection to our FastAPI IT Support service
const itSupportAgent = new HttpAgent({
  // Use environment variable for backend URL, fallback to localhost
  url: process.env.NEXT_PUBLIC_AGNO_URL || "http://0.0.0.0:8000/it-support-agent",
});
...
const runtime = new CopilotRuntime({
  agents: {
    // Our FastAPI endpoint URL for IT Support
    // @ts-ignore - Suppress TypeScript error for agent configuration
    itSupportAgent: itSupportAgent, // Register our IT Support agent
  },
});
...
export const POST = async (req: NextRequest) => {
  // Add debugging
  console.log("CopilotKit request received for IT Support agent");
  ...
  const { handleRequest } = copilotRuntimeNextJSAppRouterEndpoint({
    ...
    endpoint: "/api/copilotkit/it-support", // This API route's endpoint path
  });
