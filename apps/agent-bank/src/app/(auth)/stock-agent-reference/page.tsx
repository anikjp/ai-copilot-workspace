...
export default function StockAgentReferencePage() {
  ...
  // Catch-all action (commented out like in reference)
  // useCopilotAction({
  //   name: "*",
  ...
  useCopilotAction({
    ...
    render: (args: any) => {
      useEffect(() => {
        console.log(args, "🔵 STOCK REFERENCE: changeBackgroundColour args:", args)
      }, [args])
      return (
        <>
          <div className="flex flex-col gap-4">
  ...
  // Render standard charts and table action
  useCopilotAction({
    ...
    renderAndWaitForResponse: ({ args, respond, status }) => {
      useEffect(() => {
        console.log(args, "🔵 STOCK REFERENCE: render_standard_charts_and_table args:", args)
      }, [args])
      return (
        <>
            ...
            <>
              ...
              <div className="flex gap-2 mt-4">
                <button
                  ...
                  className="px-6 py-2 bg-green-50 text-green-700 border border-green-200 rounded-lg shadow-sm hover:bg-green-100 transition-colors font-semibold text-sm"
                  onClick={() => {
                    if (respond) {
                      // Update local state
                ...
                <button
                  hidden={status == "complete"}
                  className="px-6 py-2 bg-red-50 text-red-700 border border-red-200 rounded-lg shadow-sm hover:bg-red-100 transition-colors font-semibold text-sm"
                  onClick={() => {
                    if (respond) {
  ...
  // Render custom charts action
  useCopilotAction({
    name: "render_custom_charts",
    renderAndWaitForResponse: ({ args, respond, status }) => {
      useEffect(() => {
        console.log(args, "🔵 STOCK REFERENCE: render_custom_charts args:", args)
      }, [args])
      return (
        <>
          <LineChartComponent data={args?.investment_summary?.performanceData} size="small" />
