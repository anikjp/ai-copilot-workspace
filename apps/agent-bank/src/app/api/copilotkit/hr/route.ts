...
// STEP 1: Initialize HTTP Agent for HR Management Backend
// Create agent connection to our FastAPI HR Management service
const hrAgent = new HttpAgent({
  // Use environment variable for backend URL, fallback to localhost
  url: process.env.NEXT_PUBLIC_AGNO_URL || "http://0.0.0.0:8000/hr-agent",
});
...
const runtime = new CopilotRuntime({
  agents: {
    // Our FastAPI endpoint URL for HR Management
    // @ts-ignore - Suppress TypeScript error for agent configuration
    hrAgent: hrAgent, // Register our HR Management agent
  },
});
...
export const POST = async (req: NextRequest) => {
  // Add debugging
  console.log("CopilotKit request received for HR agent");
  ...
  const { handleRequest } = copilotRuntimeNextJSAppRouterEndpoint({
    ...
    endpoint: "/api/copilotkit/hr", // This API route's endpoint path
  });
